...
- No cross-restart audit persistence either — a `database:` key in `config.yaml` fails validation on this build (confirmed, not just undocumented), so the audit log only covers the current process's lifetime.
- `policies.cors` in `config.yaml` only affects browser callers (the gateway's own Playground, `/gateway-logs`) — native clients (`mcp` Python lib, `mcp-remote`) don't send CORS preflight at all, so it has zero effect on whether any of the 5 consumers can connect.
- **A host that "stops logging" may just have a reverted config, not a gateway bug** — `claude_desktop_config.json` got silently reset back to a direct `command: python.exe` stdio entry mid-session once already (likely Desktop's own connector UI touching the file), which routes that host straight to the Python server with zero gateway involvement — tools still work, they're just invisible to the audit log. Check the host's own config before assuming the gateway is broken.
